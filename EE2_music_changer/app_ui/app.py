...
class App(CallbackHandler):
    ...
    def __init__(self, file_pickers: Tuple[ft.FilePicker, ft.FilePicker]):
        ...
        self.game_file_picker.on_result = lambda pick_event: self.file_picker_callback(
            pick_event,
            self.game_path_input
        )
        self.custom_file_picker = file_pickers[1]
        self.custom_file_picker.on_result = lambda pick_event: self.file_picker_callback(
            pick_event,
        ...
        self.change_button = ft.ElevatedButton(
            ...
            tooltip="Changes the original music to a custom one.\n"
                    "Path to the game folder and the custom folder will be set automatically."
        )
        ...
        self.dark_mode_checkbox = ft.Checkbox(
            ...
            on_change=lambda event: self.app_theme_checkbox_callback(
                ...
                [self.game_path_button, self.custom_path_button]
            ),
        ...
        self.game_path_button = ft.ElevatedButton(
            ...
            on_click=lambda event: self.game_path_button_callback(event, self.game_file_picker),
            style=ft.ButtonStyle(
                padding=self.TEXT_BUTTON_DARK_MODE_PADDING,
                shape={
        ...
        self.custom_path_button = ft.ElevatedButton(
            text="Select path...",
            icon=ft.icons.FOLDER_OPEN,
            on_click=lambda event: self.custom_path_button_callback(event, self.custom_file_picker),
            style=ft.ButtonStyle(
                padding=self.TEXT_BUTTON_DARK_MODE_PADDING,
                shape={
                    ft.MaterialState.DEFAULT: ft.RoundedRectangleBorder(radius=self.TEXT_BUTTON_BORDER_RADIUS),
